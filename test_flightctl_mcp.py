...
class TestConfiguration:
    ...
    def test_config_file_parsing(self):
        ...
        config_data = {
            'service': {
                'server': 'https://api.flightctl.example.com',
        ...
        try:
            with patch.object(Configuration, '__init__', lambda self: None):
                ...
                assert config.insecure_skip_verify is True
                assert config.oidc_token_url == 'https://auth.flightctl.example.com/realms/flightctl/protocol/openid-connect/token'
                assert config.client_id == 'flightctl'
...
class TestCLI:
    ...
    def test_download_success(self, mock_tempdir, mock_makedirs, mock_chmod, 
                             mock_move, mock_run, mock_which):
